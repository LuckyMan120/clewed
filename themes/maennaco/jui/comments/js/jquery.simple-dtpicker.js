...
(function($) {
    ...
    /* Set a picker date based on string date */
    PickerHandler.prototype.setDateStr = function(value, insights_arr){
        var $picker = this.$pickerObject;
        var picker_id = $picker.data('pickerId');
        var date_arr = value.split(" ");
        hours = date_arr[1].split(":");
        date = date_arr[0].split("-");
        draw($picker, {
                "isAnim": false,
                "isOutputToInputObject": true
        }, date[2], date[0]-1, date[1], hours[0], hours[1],insights_arr);
    ...
    var draw = function($picker, option, year, month, day, hour, min, insights) {

        if (typeof insights !== 'undefined') {
            insights = insights;
            $picker.data('insights', insights);
        } else insights = $picker.data('insights');
        ...
        if ((!isFutureOnly || !isCurrentMonth)
            ...
            $link_before_month.click(function() {
                beforeMonth($picker);
                var date_new = getPickedDate($picker);
                MyDateString = ('0' + (date_new.getMonth()+1)).slice(-2)  + '-'
                    + ('0' + date_new.getDate()).slice(-2) + '-'
                    + date_new.getFullYear();
                creator = $(".ins_picker").attr('creator');
                window.changeTimeTable(MyDateString,creator);
            });
        ...
        for (var zz = 0; i < cellNum; i++) {
            ...
            var currDate =  new Date(date.getFullYear(), date.getMonth() + 1, realDay);
            ...
            var ins_pos = getInsightByDate(insights,currDateStr);
            var date_class = '';
            if (ins_pos != '-1') {

                if ($picker.data('insights')[ins_pos].approved == 1) {date_class = 'approved';ttip = 'Confirmed: ';}
                else {date_class = 'saved'; ttip = 'Tentative: ';}

                $td.addClass(date_class);
                $td.attr('data-tooltip',ttip + $picker.data('insights')[ins_pos].title);

            }

            if (isCurrentMonth && realDay == todayDate.getDate()) {//* today *//*
                $td.addClass('today');
                $td.attr('data-tooltip','Today');
            }
            ...
            if (
                ...
                $td.click(function() {
                    ...
                    MyDateString = ('0' + (targetDate.getMonth()+1)).slice(-2)  + '-'
            ...
            while( hour*100+min < maxTime[0]*100+maxTime[1] ){
                ...
                if (
                    ...
                    $o.click(function() {
                        ...
                        var time_id = hour*4 + Math.floor(min/15);

                        for (i=0;i<4;i++) {

                            if ($("td[rel=time"+(time_id+i)+"]").hasClass('approved')) {
                        ...
                        for (i=0;i<4;i++) {
                            ...
                            $("td[rel=time"+(time_id+i)+"]").addClass('bCurrent');
    ...
    var getInsightByDate = function(obj, date) {
        var returnKey = -1;
    ...
    var init = function($obj, opt) {
        ...
        $picker.data('insights', opt.insights);
    ...
    var methods = {
        ...
        setDateStr : function(value,insights_arr ) {
            var $input = $(this);
            var $picker = $(PickerObjects[$input.data('pickerId')]);
            if ($picker != null) {
                var handler = new PickerHandler($picker, $input);
                handler.setDateStr(value,insights_arr)
            }
        }
    ...
    $.fn.handleDtpicker = function( method,value,insights_arr ) {
        if ( methods[method] ) {
            if (method == 'setDateStr') return methods['setDateStr'].apply(this,new Array(value,insights_arr));
