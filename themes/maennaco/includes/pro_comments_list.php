	...
	function printComments($resultSet)
	{
		...
		if ($uType == 'people' || $uType == 'admin')
		{
			...
			if (file_exists($base_url.'sites/default/images/profiles/50x50/'.$crId.'.jpg'))
			{ 
				$avatar = $base_url.'sites/default/images/profiles/50x50/'.$crId.'.jpg';
			} 
			else 
			{
				if ($gender == 'm' || $gender == '') 
	...
	else if ($uType == 'company')  
	{
			...
			if (file_exists($base_url.'sites/default/images/company/50x50/'.$crId.'.jpg')) 
			{
				$avatar = $base_url.'sites/default/images/company/50x50/'.$crId.'.jpg';
			} 
			else	
			if ($cmp_role == 'service') 
				$avatar =$base_url.'/themes/maennaco/images/cmp-avatar-service.png';
			else
				$avatar =$base_url.'/themes/maennaco/images/cmp-avatar-product.png';
	...
	?>
			   ...
			   <span style="color:#4169AF;"><b><?php if (ifAdmin($row['f_name'])) {echo "MAENNA";} else echo $row['f_name']; echo "</b>
	...
	?>
							<label style="width:85%;" class="postedComments">
								<span><b><?php if (ifAdmin($rows['user'])) {echo "MAENNA";} else  echo $rows['f_name'];
				...
				*/
				...
				<div style="margin-bottom: 10px;" class="commentBox" align="right"
	...
	else
	{
		  ...
		  if (ifAdmin($user->name)) $result1 = mysql_query("SELECT * FROM maenna_company_events mce WHERE  companyid = '".$_REQUEST['id']."' ORDER BY eventid DESC"); else
		  $result1 = mysql_query ("SELECT * 
					     ...
					     ) AND companyid = '".$_REQUEST['id']."' ) OR (postedby = '".$user->uid."' AND companyid = '".$_REQUEST['id']."') ORDER BY eventid DESC ");
	 ...
	 <div id="clear" style="clear:both"></div>
		  ...
		  <div class="event-info">
			   ...
			   if (mysql_num_rows($invAll) > 0) {
				    ...
				    if (substr($proId,0,7) == 'invalid') {echo "MAENNA ADMIN &nbsp;&nbsp;";} else echo "<a class=\"tool\" target=\"_blank\" href=\"/account?tab=professionals&page=pro_detail&id=".$inv['uid']."&closebtn=1\">".$proId."</a> &nbsp;&nbsp;";
			   ...
			   while ($att = mysql_fetch_array($attres)) {
				    ...
				    if (substr($proId,0,7) == 'invalid') {echo "MAENNA ADMIN &nbsp;&nbsp;";} else echo "<a class=\"tool\" target=\"_blank\" href=\"/account?tab=professionals&page=pro_detail&id=".$att['uid']."&closebtn=1\">".$proId."</a> &nbsp;&nbsp;";
			   }
			   echo "<br>";
			   }
			   ...
			   if ($invstatus['status'] == 'sent') echo "<span id=\"att".$row1['eventid']."\"  class='attendance'>CONFIRM YOUR ATTENDANCE! <a class = \"rsvp\" name=\"confirmatt\" eventid = \"".$row1['eventid']."\" style=\"color:#6792D0 !important; cursor:pointer;\" >RSVP</a></span>";
			   
			   elseif ($invstatus['status'] == 'confirmed') echo "<span id=\"att".$row1['eventid']."\"  class='attendance'>YOUR STATUS: <a class = \"rsvp\" name=\"confirmatt\" eventid = \"".$row1['eventid']."\" style=\"color:#6792D0; cursor:pointer;\" >CONFIRMED</a></span>";
			   
			   elseif ($invstatus['status'] == 'maybe') echo "<span id=\"att".$row1['eventid']."\" class='attendance'>YOUR STATUS: <a class = \"rsvp \" name=\"confirmatt\" eventid = \"".$row1['eventid']."\" style=\"color:#6792D0; cursor:pointer;\" >TENTATIVE</a></span>";
			   
			   elseif ($invstatus['status'] == 'declined') echo "<span id=\"att".$row1['eventid']."\"  class='attendance'>YOUR STATUS: <a class = \"rsvp \" name=\"confirmatt\" eventid = \"".$row1['eventid']."\" style=\"color:##6792D0; cursor:pointer;\" >DECLINED</a></span>";
			   }
			   echo '</span><div id="clear" style="clear:both"></div>';
				    ...
				    echo "<a target='_blank' class= \"tool\" style=\"color:#6792D0;\" href='/account?tab=companies&page=company_detail&id=".$filesres['companyid']."&mtab=file&file=".urlencode('/sites/default/files/'.$filesres[data_value2])."&name=".urlencode($filesres[data_value])."'>".strtoupper($filesres[data_value])."</a> &nbsp;&nbsp; ";
				    }
			   }
...
<?php } ?>
	...
	<?php
		  if (ifAdmin($postedUname['name'])) {echo "MAENNA";} else echo strtoupper($postedUname['name']);?><?="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".strtoupper(date("M j ",$row1['created']));?></span>
