...
if ($_REQUEST['value']) {
    ...
    if (md5($_REQUEST['uid'] . $_REQUEST['fid'] . "kyarata75") === $_REQUEST['m']) {
        ...
        if (isset($_REQUEST['bEdit']) && $_REQUEST['bEdit'] == "true") {
            ...
            mysql_query("UPDATE pro_wall_posts SET post = '" . mysql_real_escape_string($_REQUEST['value']) . "',user = '" . mysql_real_escape_string($_REQUEST['uid']) . "',date_created = '" . time() . "' WHERE pid = '" . mysql_real_escape_string($_REQUEST['pid']) . "' ") or die(mysql_error());
        ...
        mysql_query("INSERT INTO pro_wall_posts (pro_id,post,f_name,user,flag,date_created) VALUES('" . checkValues($_REQUEST['fid']) . "','" . mysql_real_escape_string($_REQUEST['value']) . "','" . $editorname . "','" . $editor . "','" . $flag . "','" . time() . "')")
