...
{
	$user_id = $_REQUEST['analysis_id'];
	$pagename = '';
	$uid = $_REQUEST['analysis_id'];
}
?>
...
{ ?><li>
	<a href="<?=$base_url?>/account?tab=companies&page=company_detail&id=<?=$_REQUEST['id']?>&mtab=analysis&sort=<?=$resCategories['tags']?>"><?=$resCategories['tags']?></a></li>
<?php }
?></ul></div>
...
{
?><li>
<a href="<?=$base_url?>/account?tab=companies&page=company_detail&id=<?=$_REQUEST['id']?>&mtab=analysis&sortmonth=<?=$key?>"><?=$value?></a></li>
<?php } ?></ul></div>
</div>

<div class="left_border" style="background:#ebecec;margin-bottom:2px;"><span><a class="likess" href="<?=$base_url?>/account?tab=companies&page=company_detail&id=<?=$_REQUEST['id']?>&mtab=analysis&sortby=likes" style="cursor:pointer;"><strong style="font-style:italic;">Likes</strong></a></span></div>
