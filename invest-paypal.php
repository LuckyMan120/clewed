...
//        list($email, $firstname, $lastname) = \Clewed\Db::get_instance()->get_row($sql, array(':uid' => $user->uid));
        ...
        $custom = base64_encode(
            serialize(
                array(
                    'user_id' => $user->uid,
                    'pro_id'  => $this->data['eventId'],
                )
            )
        );
        $itemName = 'Insight';
        $itemNumber = $this->data['eventId'];
        $companyid = $this->data['eventId'];
        $amount = $this->data['amount'];
//        $email = $this->data['paypal_link'];
//        if ($this->response['price'] === 'discount') {
//            $discountAmount = $this->response['cost'];
//        }
        $returnUrl = sprintf(
            '/account?tab=companies&page=company_detail&id=%d&section=company_name&panel=invest_now',
            $companyid
        );
        ob_start();
...
$pc = new PayPalInvest($user, $_POST);
//$pc->validate();
